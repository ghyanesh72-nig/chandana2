...
const bcrypt = require('bcrypt'); // Optional: for password hashing (you should hash passwords in real-world apps)

const app = express();
const port = process.env.PORT || 5000; // Make sure the port is dynamic for Render deployment
...
app.post('/admin', async (req, res) => {
  ...
  try {
    ...
    if (admin) {
      const isValidPassword = await bcrypt.compare(password, admin.password); // Compare hashed password
      if (isValidPassword) {
